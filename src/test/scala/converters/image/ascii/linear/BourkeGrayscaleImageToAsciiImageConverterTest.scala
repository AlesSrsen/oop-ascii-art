...
class BourkeGrayscaleImageToAsciiImageConverterTest
    extends FunSuite
    with TestWithImages
    with BeforeAndAfter {
  var converter: BourkeGrayscaleImageToAsciiImageConverter = _
  ...
  test("Convert pixels to ascii") {
    ...
    assert(
      convertedImage.pixels.getPixelSeq.flatten.map(_.symbol) == Seq('$', 'q',
        'n', 'x', '_', ' '))
  }
}
